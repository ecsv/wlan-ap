...
void vif_state_dhcp_allowlist_get(struct schema_Wifi_VIF_State *vstate, char *section)
{
	char fqdn[32];
	char read_ifname[8];
	char set[8];
	struct blob_attr *td[__DNS_ATTR_MAX] = { };
	uci_load(dns, "dhcp", &dhc);
	ip_section = uci_lookup_section(dns, dhc, section);
	if(!ip_section) {
...
/*bool isIpAddresscorrect(char *ipAddress)
{
	struct sockaddr_in sa;
	int result = inet_pton(AF_INET, ipAddress, &(sa.sin_addr));
	//LOGN("Hi result:%d", result);
	return result != 0;
}
int ipset_add(char *ipaddress)
{
	char command[64];

	sprintf(command,"ipset add /set_wlan1 %s", ipaddress);
...
void vif_dhcp_opennds_allowlist_set(const struct schema_Wifi_VIF_Config *vconf, char *ifname)
{
	...
	blob_to_uci_section(dns, "dhcp", "@dnsmasq[0]", "dnsmasq", dnsmas.head, &dnsm_param, NULL);
	uci_commit_all(dns);
	return;
}
